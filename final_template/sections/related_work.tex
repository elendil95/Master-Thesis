...
Key space can be theoretically infinite, but in practice it is bound by both the alphabet and the key length: in the case of an hexadecimal key of length 10, the key space can be quantified as $16^{10}$ possibilities; on the other hand in the case of the password \texttt{password} (written with lowercase characters only), the key space can be expressed as $26^8$. %convert key space sizes to power of 2 to show the key length and compare.
...
Cryptographic hash functions adhere to certain security standards (hence the adjective \enquote{cryptographic}) but they are different from encryption algorithms: %like AES or PGP:
